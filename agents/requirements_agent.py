    ...
    def _build_regulator_thermal_rows(self, comps, rail_totals_typ, rails, rail_labels,
        ...
        def _scan_vout(c):
            blob = (f"{c.get('function','')} {c.get('primary_description','')} "
                    f"{c.get('primary_part','')}").lower()
            # Strip input-voltage phrases so "12 V input" doesn't leak in
        ...
        for i, c in enumerate(comps, 1):
            if not _is_reg(c):
                continue
            topo = _topology(c)
            vin  = _spec_num(c, ("input_voltage", "vin", "input_voltage_range",
                                  "v_in", "supply_voltage_range"), "V") or sys_vin
            vout = _spec_num(c, ("output_voltage", "vout", "v_out",
                                  "supply_voltage"), "V")
            if vout <= 0:
...
"""
    ...
    def _glb_sanity_flags(self, stage) -> list:
        ...
        if any(k in lbl for k in ("lna", "amp", "amplifier", "driver", "(pa)", " pa")) \
    ...
    def _glb_frequency_sweep(self, glb: dict):
        ...
        _ACT_KW  = ("lna", "amp", "amplifier", "driver", " pa", "(pa)",
    ...
    def _glb_contract_checks(self, glb_raw: dict, glb_proj: dict, bom: list) -> list:
        ...
        _PASSIVE_KW = _FILT_KW + (
            ...
            "attenuator", "pad", "splitter", "combiner", "coupler",
            "balun", "transformer", "isolator", "circulator", "limiter",
            "diode",
        )
        ...
        def _is_active(st):
            ...
            if any(kw in lbl for kw in _FILT_KW):
                return False
            g = st.get("gain_db")
            if any(kw in lbl for kw in _ACT_KW):
                return True
    ...
    def _build_gain_loss_budget_md(self, tool_input: dict, project_name: str) -> str:
        ...
        # ── Power Consumption per Stage (bias conditions from datasheet) ────
        # Every gain / NF / P1dB number in the stage table is only valid
        # under the datasheet bias condition listed here. The bias current
        # and drain voltage come from the same datasheet row — otherwise
        # the specified RF performance is not achievable. These Pdc values
        # must match what the Power Budget document uses per rail.
        _bias_rows = []
        _total_pdc = 0.0
        for i, st in enumerate(glb_raw.get("stages", []), 1):
            bc = st.get("bias_conditions") or {}
            vdd = bc.get("vdd_v")
            idq = bc.get("idq_ma")
            pdc = bc.get("pdc_mw")
            cond = bc.get("datasheet_condition") or ""
            # Derive Pdc from Vdd × Idq if the LLM provided both but
            # omitted the product, or sanity-check the provided product.
            if isinstance(vdd, (int, float)) and isinstance(idq, (int, float)):
                pdc_calc = round(vdd * idq, 1)  # V × mA = mW
                if not isinstance(pdc, (int, float)):
                    pdc = pdc_calc
                elif abs(pdc - pdc_calc) > 10.0:   # > 10 mW mismatch
                    cond = (cond + f" · Pdc mismatch (Vdd·Idq = {pdc_calc} mW)").strip(" ·")
            if isinstance(pdc, (int, float)):
                _total_pdc += pdc
            _bias_rows.append((i, st, vdd, idq, pdc, cond))

        if any(isinstance(row[4], (int, float)) for row in _bias_rows):
            lines += [
                f"## {sec}. Power Consumption per Stage — Datasheet Bias Conditions",
                "",
                "| # | Stage | Component | Vdd (V) | Idq (mA) | Pdc (mW) | Datasheet Condition |",
                "|---|-------|-----------|--------:|---------:|---------:|---------------------|",
            ]
            for i, st, vdd, idq, pdc, cond in _bias_rows:
                # Passive / non-powered stages: show em-dashes, no row hidden
                ...
                cond_s = cond if cond else ""
                lines.append(
                    f"| {i} | {st.get('stage_name', '—')} | {st.get('component', '—')}"
                    f" | {vdd_s} | {idq_s} | {pdc_s} | {cond_s} |"
                )
            lines.append(
        ...
        if stages:
            _FILT_KW = ("filter", "bpf", "lpf", "hpf", "diplexer", "duplexer",
                        "preselector", "saw", "baw", "cavity")
            _ACT_KW2 = ("lna", "amp", "amplifier", "driver", " pa", "(pa)",
                        "mixer", "modulator", "demodulator", "vga", "vca")
            lines += [
...
"""
        ...
        # ── Power Consumption per Stage (bias conditions) ───────────────────
        # Bias values come from the same datasheet row as gain / NF / P1dB.
        # The Pdc column must match the per-rail currents in the Power Budget.
        bias_html = ""
        _bias_rows = []
        _total_pdc = 0.0
        for i, st in enumerate(glb_raw.get("stages", []), 1):
        ...
        if any(isinstance(r[4], (int, float)) for r in _bias_rows):
            ...
            bias_html = f"""
<h2>Power Consumption per Stage — Datasheet Bias Conditions</h2>
<table>
  <tr><th>#</th><th>Stage</th><th>Component</th>
...
in the stage-by-stage table. Different bias ⇒ different RF performance. The <b>Pdc per
stage</b> (mW) flows directly into the Power Budget document — these numbers must match
the per-rail current draws in that spreadsheet. Example: HMC8410 LNA — 15 dB typ gain
is specified at Vdd = 5.0 V, Idq = 65 mA (Pdc = 325 mW). Biasing it at 3.3 V / 40 mA
reduces gain to ≈ 13 dB and degrades NF by ~0.3 dB — the GLB numbers would no longer
be valid.</p>
"""
